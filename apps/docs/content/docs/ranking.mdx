...
- **Retired** - the provider no longer serves the model, so it's out of rotation.
  Its rating is preserved from past votes.
- **Suspended** - the model was pulled for breaking the
  [integrity policy](/integrity). It sits at the bottom of the board, unranked,
  with the reason and date. Its votes and history are kept, the label doesn't
  expire, and a suspended model can't be retracted or renamed.
